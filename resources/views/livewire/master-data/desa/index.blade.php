<div class="flex h-full w-full flex-1 flex-col gap-6">
        <div class="flex items-center justify-between">
            <div>
                <flux:heading size="xl">Master Desa</flux:heading>
                <flux:heading size="sm" class="mt-2 text-zinc-600 dark:text-zinc-400">
                    Kelola data desa - basis multi-tenancy sistem SIPKUD
                </flux:heading>
            </div>
            <flux:button :href="route('desa.create')" variant="primary" wire:navigate>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-zinc-200 dark:border-zinc-700">
                            ...
                            <th class="px-4 py-3 text-left text-sm font-semibold">Nama Desa</th>
                    ...
                    <tbody>
                        @forelse($desa as $item)
                            <tr class="border-b border-zinc-200 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-900">
                                <td class="px-4 py-3 text-sm">{{ $desa->firstItem() + $loop->index }}</td>
                                <td class="px-4 py-3 text-sm font-mono">{{ $item->kode_desa }}</td>
                                <td class="px-4 py-3 text-sm">{{ $item->nama_desa }}</td>
                                <td class="px-4 py-3 text-sm">{{ $item->kecamatan->nama_kecamatan }}</td>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <flux:button 
                                            ...
                                            wire:confirm="Apakah Anda yakin ingin menghapus desa ini?"
                                            variant="ghost" 
                                            size="sm"
